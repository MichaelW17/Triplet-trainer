# 10/17/2019: M.F. predictor.py, 计算三重损失模型inference得到的各类的类内距离和类间距离
...
def create_high_dim_embs(dataset_path, classes, x_mean, avg_num):
    embs = np.ones((len(classes), emb_dim))
    ...
    for i in classes:
        ...
        print('intra class distance:', np.trace(np.cov(predictions.T)))
        embs[i] = np.sum(predictions, axis=0) / 10
...
# 求embs的协方差矩阵的迹
print('inter class distance:', np.trace(np.cov(embs.T)) * 2)

print('embs.shape: ', embs.shape)
print('np.max(embs, axis=1)', np.max(embs, axis=1))
print('np.argmax(embs, axis=1)', np.argmax(embs, axis=1))
