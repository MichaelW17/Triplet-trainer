...
# emb length
def mean_length(y_true, y_pred):
    return tf.reduce_mean(tf.norm(y_pred, ord='1', axis=1))  # L1 norm


# 02/20/2020: new metrics
# 1: batch中loss大于0的三元组的数量，训练越充分，这个值越小
def positives(labels, embeddings, margin=margin_config, squared=False):
    """
    Args:
...
# 2: 类间距离/类内距离
def d_ratio(labels, embeddings, squared=False):
    # Get the pairwise distance matrix
